...
class TodoList:
    ...
    def add(self, todoobject):
        if isinstance(todoobject, Todo):
            self._todos.append(todoobject)
            return

        raise TypeError('Only Todo objects can be added!')
    ...
    def mark_all_done(self):
        for index, _ in enumerate(self._todos):
    ...
    def select(self, callback):
        ...
        # feed add into the each method
        # if item contains 'y' in name, add to new list 
        def choose(todo):
            if callback(todo):
                new_list.add(todo)
    ...
    def find_by_title(self, title):
        # returns todo whose title matches title
